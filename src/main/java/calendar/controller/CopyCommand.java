...
public class CopyCommand extends CommandFactory {
   ...
   */
  public CopyCommand(String commandLine, MultiCalendarManagerInterface manager, CalendarInterface calendar) {
  ...
  protected void parseCommand(String commandLine) throws IllegalArgumentException {
    ...
    if (matcher.matches()) {
      ...
      String targetCalendarName = ((matcher.group(3) != null) ? matcher.group(3).trim() : matcher.group(4)).trim();
    ...
    if (matcher.matches()) {
      ...
      String targetCalendarName = ((matcher.group(2) != null) ? matcher.group(2).trim() : matcher.group(3)).trim();
    ...
    if (matcher.matches()) {
      String subject =  (matcher.group(1) != null) ? matcher.group(1).trim() : matcher.group(2).trim();
      String startDateTime = matcher.group(3).trim();
      String targetCalendarName = ((matcher.group(4) != null) ? matcher.group(4).trim() : matcher.group(5)).trim();
